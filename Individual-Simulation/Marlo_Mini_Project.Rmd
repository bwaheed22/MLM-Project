...
e. Do you see any real change in the $\beta_X$ estimate? **comment**
    ...
    Response: The $\beta_X$ estimate has increased slightly to `r round(coefs[2,1], 3)`, and the 95% confidence band as also slightly widened to [`r round(lower, 3)`, `r round(upper, 3)`] but still covers the truth of 1.

f. What is the total sample size $N$ used in the model fit? **comment**

    Response: The total sample size $N$ is `r scales::comma(length(summary(lme2)$residuals))` which corresponds to $p = 0.5$.
...
d. Do you see any real change in the $\beta_X$ estimate?
    ...
    Response: The $\beta_X$ estimate has decreased to `r round(coefs[2,1], 3)`, and the 95% confidence band as also slightly widened to [`r round(lower, 3)`, `r round(upper, 3)`] but still covers the truth of 1.

e. What is the total sample size $N$ used in the model fit? **comment**

    Response: The total sample size $N$ is `r scales::comma(length(summary(lme3)$residuals))` which corresponds to mean of our predictor.
...
e. Do you see any real change in the $\beta_X$ estimate? **comment**
    i. Does a 95% confidence band for this coefficient estimate cover the “truth” that you used to generate the data? **comment**
      ...
      Response: The $\beta_X$ estimate has decreased signficantly to `r round(coefs[2,1], 3)`, and the 95% confidence band as also slightly widened to [`r round(lower, 3)`, `r round(upper, 3)`] and does not cover the truth of 1.
