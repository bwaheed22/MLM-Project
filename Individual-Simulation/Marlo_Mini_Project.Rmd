...
e. Do you see any real change in the $\beta_X$ estimate? **comment**
    ...
    Response: The $\beta_X$ estimate has increased slightly to `r round(coefs[2,1], 3)`, and the 95% confidence band as tightened to [`r round(lower, 3)`, `r round(upper, 3)`].
...
```
    ...
    Response: The total sample size $N$ is `r scales::comma(length(summary(lme2)$residuals))`.
...
d. Do you see any real change in the $\beta_X$ estimate?
    ...
    Response: The slope estimate has decreased along with the confidence band: [`r round(lower, 3)`, `r round(upper, 3)`].
...
```
    ...
    Response: The total sample size $N$ is `r scales::comma(length(summary(lme3)$residuals))`.
...
e. Do you see any real change in the $\beta_X$ estimate? **comment**
    i. Does a 95% confidence band for this coefficient estimate cover the “truth” that you used to generate the data? **comment**
      ...
      Response: The slope estimate has decreased signficantly along with the confidence band: [`r round(lower, 3)`, `r round(upper, 3)`].
...
```
    ...
    Response: The total sample size $N$ is `r scales::comma(length(summary(lme4)$residuals))`.
