...
```
    ...
    Response:
  The addition of random slope on the *minority* variable, varying by schools and allowing for correlation between the slope and intercept, is not significant (at the 0.05 level of significance), according to the ANOVA LRT comparing the model with and without the random slope addition.
...
lm_slopes2 <- lmer(math1st ~ housepov + yearstea + mathknow + mathprep + sex + minority +  
    ses + (minority | schoolid) + (1 | schoolid/classid), data = classroom)
lm_slopes3 <- lmer(math1st ~ housepov + yearstea + mathknow + mathprep + sex + minority +  
    ses + (0 + ses | schoolid) + (minority | schoolid) +(1 | schoolid/classid), data = classroom)
...
```  
    ...
    Reponse:
  The complex model with both random slopes is justified, with a p-value of 0.001859.
